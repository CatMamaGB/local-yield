...
- ✅ **Audit follow-up (admin, CORS credentials, X-Request-Id, gating, tests):** Admin request-logs at `GET /api/admin/request-logs` (pagination, filters: route, method, statusMin/Max, userId, since, includeIp). CORS credentials are opt-in (`credentials: false` by default for Bearer-only mobile API). All API responses that include a requestId now set the `X-Request-Id` response header. Server-side admin gating: layout redirects signed-in non-admins to `/admin/forbidden`; proxy requires auth for `/admin/*` (except `/admin/login`) when Clerk is configured. Integration tests: `npm run test:auth-cors` (401/WWW-Authenticate, admin 401, CORS preflight, X-Request-Id).
