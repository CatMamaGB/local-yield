/**
 * Dashboard orders: buyer sees their orders (with pickup code); producer sees orders to fulfill.
 */
...
export default async function DashboardOrdersPage() {
  ...
  const orders = isProducer
    ? await getOrdersForProducer(user.id)
    : await getOrdersForBuyer(user.id);
              ...
              {orders.map((o) => (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="font-display font-semibold text-brand">{o.product.title}</p>
                      <p className="text-sm text-brand/70">
                        {o.producer.name ?? "Producer"} · Pickup {formatDate(o.pickupDate)}
                      </p>
                    </div>
                    {o.pickupCode && (
  ...
  return (
    <div className="min-h-screen bg-brand-light">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        {orders.length === 0 ? (
          <p className="mt-6 text-brand/70">No orders yet.</p>
        ) : (
          <ul className="mt-6 space-y-4">
            {orders.map((o) => (
              <li
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <p className="font-display font-semibold text-brand">{o.product.title}</p>
                    <p className="text-sm text-brand/70">
                      {o.buyer.name ?? "Buyer"} · Pickup {formatDate(o.pickupDate)}
                    </p>
                  </div>
                  {o.pickupCode && (
                    <div className="rounded-lg border-2 border-dashed border-brand/30 bg-brand-light/50 px-4 py-2 text-center">
